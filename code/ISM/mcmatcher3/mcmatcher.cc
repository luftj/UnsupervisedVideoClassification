  ...
  /*******************************************************************/
{
  ...
  if ( fMapFile.exists() == false ) {
    ...
    m_bMapsOn = false;
    OpGrayImage imgEmpty( 0,0);//m_grayImg.width(), m_grayImg.height() );
...
Detector* ISMReco::createDetector( bool bVerbose )
{
  unsigned nNewIdx = m_tblDetectors->numRows();
      ...
      for( int y = miny; y < maxy; y++ )
        for( int x = minx; x < maxx;  x++ ) {
          ...
          if(  dVal != 0.0 ) {
            ...
            QColor qcol = QColor(nColVal, nColVal, 0);
