...
const QString usage =
    ...
    -cd CBFILE OCCFILE       : only create a detector from codebooks\n \
    \n";
...
int main( int argc, char **argv )
{
  ...
  for(int i=1; i<argc; i++) {
    ...
    if (strcmp(argv[i],"-cd")==0 && argc>i+3)
    {
      ...
      ++i; // skip outfile
      if((argc-i) % 2 == 0) // remaining arguments are codebook and occurence files
        numCues = (argc-i) / 2;
      ...
      cout << " with " + to_string(numCues) + " cues." << endl;
      

      Detector* d = w.createDetector( bVerbose);
      for( int n = 0; n < numCues; ++n)
      {
        string cbfile  = argv[i  + 2*n];
        cout << cbfile << endl;
        string occfile = argv[i+1+ 2*n];
        cout << occfile << endl;

        d->addCue(cbfile,occfile,bVerbose);
      }
      d->saveDetector(outfile);
