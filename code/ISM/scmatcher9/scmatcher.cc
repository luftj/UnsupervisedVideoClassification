...
void ISMReco::saveOccurrences(QString qsFileName)
  /*******************************************************************/
  /* Save the current occurrences (using the FeatureVector file for- */
  /* mat).                                                           */
  /*******************************************************************/
...
//       hypo.nBoxHeight = height;
  ...
  if( m_parReco.params()->m_bDoMDL ) {
    ...
    for( int i=0; i<(int)vHyposMDL.size(); i++ ) {
      ...
      list<HoughVote> vSupportingList;
      vSupportingList = m_ismReco.getSupportingVotes( fvWindowPos );
      vector<HoughVote> vSupporting(vSupportingList.begin(),vSupportingList.end());
      ...
      Segmentation segNew = m_ismReco.drawSegmentationOffset( vSupportingList, 
                                                              m_vPointsInside, 
                                                              m_fcCue,
                                                              nSegOffX, 
                                                              nSegOffY,
    ...
    for( int i=0; i<(int)vResultHypos.size(); i++ ) {
      ...
      m_ismReco.drawSegmentation( vSupportingList, m_vPointsInside, m_fcCue,
                                  imgPFig, imgPGnd, imgSeg, true );
                                  ...
                                  bool bDisplayResults )
{
  ...
  for( int i=0; i<(int)vResultHypos.size(); i++ ) {
    ...
    if( bDisplayResults ) {
      ...
      if( m_bDisplaySegment || m_bDisplaySupport ) {
        list<HoughVote> vSupportingList;
        vSupportingList = m_ismReco.getSupportingVotes( fvWindowPos );
        vSupporting.assign(vSupportingList.begin(),vSupportingList.end());
  ...
  if( bDisplayResults && m_bDisplaySegment && (vRefResultImgs.size() > 0) ) {
    ...
    list<HoughVote> vAllSupportingList(vAllSupporting.begin(),vAllSupporting.end());

    m_ismReco.drawSegmentation( vAllSupportingList, m_vPointsInside, m_fcCue,
  ...
  /*******************************************************************/
{
  ...
  if( bDisplayResults ) {
    ...
    if( true ) {
      ...
      // vVotes = m_ismReco.getSupportingVotes( fvWindowPos, fvWindowSize );
      list<HoughVote> vvlist = m_ismReco.getSupportingVotes( fvWindowPos, fvWindowSize );
      vVotes.assign(vvlist.begin(),vvlist.end());
  ...
  /*******************************************************************/
{
  ...
  {
    list<HoughVote> vVotesList(vVotes.begin(), vVotes.end());
    imgResult = m_ismReco.drawVotePatches( vVotesList, m_vPointsInside, 
                                           vClusterPatches,
                                           m_fcCue, nDrawStyle, 
                                           bBackgroundZero );
  }
...
{ 
  ...
  ofstream ofSingle( qsResultFile1.ascii() );
  if( !ofSingle ) {
  ...
  ofstream ofScore( qsResultFile2.ascii() );
  if( !ofScore ) {
...
{ 
  ...
  ofstream ofInitial( qsResultFile1.ascii() );
  if( !ofInitial ) {
    cerr << "ERROR: Couldn't open file " << qsResultFile1 << "!" << endl;
    return;
  }
  ...
  ofstream ofRefined( qsResultFile2.ascii() );
  if( !ofRefined ) {
    cerr << "ERROR: Couldn't open file " << qsResultFile2 << "!" << endl;
    return;
  }
...
{ 
  ...
  if( m_bWriteDetections ) {
    ...
    ofDetect.open( qsResultFile3.ascii() );
    if( !ofDetect ) {
